...
## Version log (live eval)

**Round 2 — primitives.** Shipped: `image`, `button`, `checkbox`, `chip`, `tabs`, `search`, per-node `color`, `listRow.subText`, UI type ramp, `crossfade`+`transitionMs`, `list_icons`. Fixed P0s: `validate_deck`/`theme_import` object params; tofu-glyph cause identified.

**Round 3 — motion layer (the fork, crossed).** Shipped: container `justify`/`align` (default `start` — §4 P0 fixed); `sparkle` node + `gradient:"ai"` + `badge tone:"ai"` (§2 v1 ports); **state `focus:{scale,x,y,dim}` camera** (§3 gap 1); node `enter`/`exit` (§3 gap 2); `vector`; §5 doc bug fixed. Verified: "AI-Replied narrative with camera push-in" — previously impossible — renders natively.

**Round 4 — the freedom layer.** Shipped the open escape hatch: per-node **`at`** (absolute timing), inline **`anim`** on any node, **container-as-box** (`bg`/`border`/`radius`/`shadow`, incl. `"ai"`), `vector`. Verified: **gap 3 ("AI detecting") closed by composition** — a `box`+`sparkle`+gradient-text pill on a new `detecting` state, no new primitive.
  - 🐞 New: `bg:"ai"` + `gradient:"ai"` text = gradient-on-gradient, unreadable → force light text on an `"ai"` fill.
  - Doc: `at` contradicts the standing "don't set per-node delays" line — reconcile.

## The one remaining frontier — an OVERLAY layer
Everything except **absolute/overlay positioning** is now expressible. Still blocked, all for the same reason ("constrained flex only — no grid/absolute"):
- **cursor / click sim** (gap 4) — can't pin a pointer over a target node.
- **`focus:{target:"<nodeId>"}`** auto-framing — today's camera is hand-tuned x/y %.
- **list-insert reflow** (gap 6) — `enter` fades a row in; siblings don't push.
An overlay/anchor layer (position a node relative to another, above the flow) closes all three. That's the next build.
