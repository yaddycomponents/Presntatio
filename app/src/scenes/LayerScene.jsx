...
const topFor = (i) => (layers.length - 1 - i) * ROW
const OVERRIDES_C = topFor(3) + BARH / 2
const ANTD_TOP = topFor(1)
const FLOAT_TOP = -82
const FLOAT_C = FLOAT_TOP + BARH / 2
const CHIPW = 300
const H = topFor(0) + BARH
...
export default function LayerScene() {
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '2.4vw' }}>
      ...
      <div style={{ position: 'relative', width: BARW, height: H, marginTop: '0.5vw' }}>
        {layers.map((name, i) => (
          <motion.div
            key={name}
        ...
        <motion.div
          initial={{ top: FLOAT_TOP, opacity: 0, left: (BARW - CHIPW) / 2 }}
          animate={{
            top: [FLOAT_TOP - 30, FLOAT_TOP, FLOAT_TOP, ANTD_TOP, ANTD_TOP],
            opacity: [0, 1, 1, 1, 1],
        ...
        >
          ...
          <motion.span animate={{ opacity: [0, 1, 1, 0, 0] }} transition={{ delay: 2.9, duration: 2.4, times: [0, 0.12, 0.6, 0.72, 1] }} style={{ fontFamily: tokens.font.mono, fontSize: 9, letterSpacing: '0.1em', textTransform: 'uppercase', color: tokens.data.beforeNum, border: `1px solid ${tokens.data.beforeNum}`, borderRadius: 3, padding: '1px 5px' }}>unlayered ✕</motion.span>
          <motion.span animate={{ opacity: [0, 0, 0, 1, 1] }} transition={{ delay: 2.9, duration: 2.4, times: [0, 0.6, 0.78, 0.9, 1] }} style={{ fontFamily: tokens.font.mono, fontSize: 9, letterSpacing: '0.1em', textTransform: 'uppercase', color: tokens.data.afterNum, border: `1px solid ${tokens.data.afterNum}`, borderRadius: 3, padding: '1px 5px' }}>StyleProvider ✓</motion.span>
        </motion.div>

        <motion.div
          initial={{ top: OVERRIDES_C - 13, opacity: 0 }}
          animate={{ top: [OVERRIDES_C - 13, OVERRIDES_C - 13, FLOAT_C - 13, FLOAT_C - 13, OVERRIDES_C - 13], opacity: [0, 1, 1, 1, 1] }}
          transition={{ delay: 1.7, duration: 4.0, times: [0, 0.30, 0.38, 0.74, 0.82], ease: ease.inOut }}
          style={{ position: 'absolute', left: BARW + 22, width: 84, height: 26, borderRadius: 999, background: tokens.data.afterNum, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}
        >
          <span style={{ fontFamily: tokens.font.mono, fontWeight: 700, fontSize: 11, letterSpacing: '0.08em', textTransform: 'uppercase', color: tokens.bg.primary }}>✓ wins</span>
        </motion.div>

        <motion.div
          animate={{ opacity: [0, 1, 1, 0] }}
          transition={{ delay: 3.1, duration: 1.5, times: [0, 0.18, 0.78, 1] }}
          style={{ position: 'absolute', top: H + 8, left: 0, width: BARW, display: 'flex', justifyContent: 'center', gap: 10 }}
        >
          {['clear-icon', 'skeleton overflow', 'washed-out tags'].map((s) => (
            <span key={s} style={{ fontFamily: tokens.font.mono, fontSize: 9, letterSpacing: '0.05em', textTransform: 'uppercase', color: tokens.data.beforeNum, border: `1px solid ${tokens.data.beforeNum}`, borderRadius: 3, padding: '2px 6px' }}>{s}</span>
          ))}
        </motion.div>
      </div>

      <div style={{ position: 'relative', width: 'min(720px, 92vw)', height: 30, marginTop: '1.4vw' }}>
