...
import { Sparkle, Grad, Mask, fadeS, riseS, sec, EASE_OUT } from '../anim.jsx'
import { Eyebrow, AiBadge } from '../ui.jsx'
import { Thread, Msg, Attach, Composing, Sent } from '../mail.jsx'

const clamp = { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }

export const ShiftR = () => {
  const f = useCurrentFrame()
  const composeOp = interpolate(f, [sec(1.7), sec(1.86), sec(2.5), sec(2.8)], [0, 1, 1, 0], clamp)
  const strike = interpolate(f, [sec(3.8), sec(4.2)], [0, 1], { ...clamp, easing: EASE_OUT })
  return (
    <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '3.2vh' }}>
      ...
      <Thread subject="Re: Copy of invoice INV-3221" w={840} delay={0.5}>
        ...
        <Msg ai name="Growfin AI" time="10:11 AM" delay={1.5} badge={<AiBadge label="AI Replied" delay={2.7} />}>
          <div style={{ position: 'relative', minHeight: 96 }}>
            <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', gap: 12, color: t.muted, fontSize: 15, opacity: composeOp }}>
              <Composing delay={1.7} /> Generating a grounded reply…
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14, ...fadeS(f, 2.7) }}>
              <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginTop: 2 }}>
                ...
                <span style={{ fontFamily: t.mono, fontSize: 13, color: t.muted, ...fadeS(f, 3.35) }}>no human touch</span>
              </div>
            </div>
          </div>
        </Msg>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, ...riseS(f, 3.5, 12) }}>
        ...
        <span style={{ position: 'relative', fontFamily: t.mono, fontSize: 16, color: t.faint }}>
          2–9 hours
          <span style={{ position: 'absolute', left: 0, right: 0, top: '52%', height: 2, background: t.danger, transform: `scaleX(${strike})`, transformOrigin: 'left' }} />
        </span>
        ...
        <span style={{ fontFamily: t.mono, fontSize: 'clamp(18px,1.8vw,24px)', fontWeight: 700, background: t.ai, WebkitBackgroundClip: 'text', backgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>under 60 seconds</span>
      </div>
    </AbsoluteFill>
  )
}
