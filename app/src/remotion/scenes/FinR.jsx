...
export const FinR = () => {
  ...
  const growOp = interpolate(f, [0, sec(0.3), sec(0.9), sec(1.35)], [0, 1, 1, 0], { ...clamp, easing: EASE_INOUT })
  const growY = interpolate(f, [0, sec(0.3), sec(0.9), sec(1.35)], [16, 0, 0, 64], { ...clamp, easing: EASE_INOUT })
  const finOp = interpolate(f, [0, sec(0.3), sec(1.5), sec(1.85)], [0, 1, 1, 0], { ...clamp, easing: EASE_INOUT })
  // phase 2 — lone fin
  const loneOp = interpolate(f, [sec(2.1), sec(2.8), sec(3.9), sec(4.7)], [0, 1, 1, 0], { ...clamp, easing: EASE_OUT })
  const loneSc = interpolate(f, [sec(2.1), sec(2.8)], [0.96, 1], { ...clamp, easing: EASE_OUT })
  const ulSc = interpolate(f, [sec(2.4), sec(2.9), sec(3.9), sec(4.6)], [0, 1, 1, 0], { ...clamp, easing: EASE_INOUT })
  const ulOp = interpolate(f, [sec(2.4), sec(2.9), sec(3.9), sec(4.6)], [0, 1, 1, 0], { ...clamp, easing: EASE_INOUT })
