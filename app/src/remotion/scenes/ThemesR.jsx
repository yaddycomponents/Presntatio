...
import { Sparkle, Mask, riseS } from '../anim.jsx'
import { Eyebrow, Toggle, IconChip } from '../ui.jsx'
...
function ThemeCard({ Icon, title, desc, delay, toggleAt }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '18px 20px', background: t.panel, border: `1px solid ${t.border}`, borderRadius: 14, boxShadow: t.shadowSoft, ...riseS(f, delay, 18) }}>
  ...
  return (
    <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '3vh' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18, width: 'min(960px, 92vw)' }}>
        {themes.map((th, i) => <ThemeCard key={th.title} {...th} delay={0.7 + i * 0.12} toggleAt={1.7 + i * 0.13} />)}
      </div>
      <div style={{ fontSize: type.label, color: t.muted, display: 'flex', alignItems: 'center', gap: 10, ...riseS(f, 2.8, 12) }}>
