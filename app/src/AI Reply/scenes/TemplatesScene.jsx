...
import { Sparkle, Grad, Bar, Toggle, fade, rise, pop } from '../fx'
import { Caption } from '../app'

const themes = [
  { Icon: ReceiptText, on: true },
  { Icon: FileText, on: true },
  { Icon: FileSpreadsheet, on: true },
  { Icon: Landmark, on: true, active: true },
  { Icon: Files, on: true },
  { Icon: MessagesSquare, on: true },
]

function ThemeRow({ Icon, on, active, delay }) {
  return (
    <motion.div {...fade({ delay })}
      style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 14px', borderRadius: 12, border: `1px solid ${active ? t.primaryLine : t.border}`, background: active ? t.primarySoft : t.panel }}>
...
function Token({ children, delay }) {
  return (
    <motion.span {...pop({ delay })}
      style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '3px 9px', borderRadius: 7, background: 'rgba(29,57,196,0.09)', color: t.aiBlue, fontSize: 14.5, fontWeight: 500, verticalAlign: 'middle' }}>
...
export default function TemplatesScene() {
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex' }}>
      ...
      <div style={{ flex: 1, padding: '26px 34px', display: 'flex', flexDirection: 'column', gap: 16, minWidth: 0 }}>
        ...
        {/* toolbar */}
        <motion.div {...fade({ delay: 0.7 })} style={{ display: 'flex', alignItems: 'center', gap: 18, padding: '12px 16px', border: `1px solid ${t.border}`, borderRadius: 10, background: t.panelAlt }}>
          {toolbarIcons.map((Ic, i) => <Ic key={i} size={16} color={t.muted} strokeWidth={2} />)}
        </motion.div>
        {/* body */}
        <motion.div {...rise({ delay: 0.9, y: 12 })}
          style={{ flex: 1, border: `1px solid ${t.border}`, borderRadius: 12, padding: '22px 24px', display: 'flex', flexDirection: 'column', gap: 18, fontSize: 15.5, color: '#33384a', lineHeight: 1.7 }}>
      ...
      <div style={{ width: 360, borderLeft: `1px solid ${t.border}`, background: t.canvas, padding: '26px 28px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <motion.div {...fade({ delay: 2.3 })} style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        <motion.div {...fade({ delay: 2.5 })} style={{ fontSize: 14.5, color: '#33384a', lineHeight: 1.65, display: 'flex', flexDirection: 'column', gap: 14 }}>
